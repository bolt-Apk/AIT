...
async function sendEmailAlert(adminEmail: string, failedModels: ModelCheck[]) {
  ...
  const html = `
    ...
    <p style="color:#64748b;font-size:12px;margin-top:16px">Avirond Model Monitor</p>
  `;
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Avirond Monitor <monitor@avirond.com>",
        to: adminEmail,
        subject: `[Avirond] ${failedModels.length} модел${failedModels.length === 1 ? "ь" : failedModels.length < 5 ? "и" : "ей"} не работа${failedModels.length === 1 ? "ет" : "ют"}`,
        html,
      }),
    });
  } catch {
